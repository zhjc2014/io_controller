...
#define EN_ACK_WAIT         0
#define ACK_TIMOUT_THR      ARD_4000US  //wait time
#define REPEAT_CNT          5		    //重复次数

#define MAX_CHANNEL_NUM     6
#define MAX_PACKAGE_LEN		32
#define DEFAULT_BASE_ADDR   0x11, 0x22, 0x33, 0x44
#define DEFAULT_CHANNEL     0x05


/** RF24L01硬件接口定义 */
#define RF24L01_POW_EN_PORT				GPIOB
#define RF24L01_POW_EN__CLK				RCC_APB2Periph_GPIOB
#define RF24L01_POW_EN_PIN				GPIO_Pin_10
...
/** 口线操作函数定义 */
#define RF24L01_SET_POWER_ON()			RF24L01_POW_EN_PORT->ODR &= (uint32_t)( ~(uint32_t)RF24L01_POW_EN_PIN )	
#define RF24L01_SET_POWER_OFF()			RF24L01_POW_EN_PORT->ODR |= RF24L01_POW_EN_PIN
...
#define RD_RX_PLOAD     0x61	//读RX有效数据�?~32字节
#define WR_TX_PLOAD     0xA0	//写TX有效数据�?~32字节
...
#define CONFIG          0x00	//配置寄存器地址，bit0:1接收模式,0发射模式;bit1:电选择;bit2:CRC模式;bit3:CRC使能;
							    //bit4:中断MAX_RT(达到最大重发次数中�?使能;bit5:中断TX_DS使能;bit6:中断RX_DR使能	
...
#define STATUS          0x07	//状态寄存器;bit0:TX FIFO满标�?bit1~3:接收数据通道�?最�?6);bit4:达到最多次重发次数
								//bit5:数据发送完成中�?bit6:接收数据中断
#define MAX_TX  		0x10	//达到最大发送次数中�?
#define TX_OK   		0x20	//TX发送完成中�?
#define RX_OK   		0x40	//接收到数据中�?

#define OBSERVE_TX      0x08	//发送检测寄存器,bit7~4:数据包丢失计数器;bit3~0:重发计数�?
#define CD              0x09	//载波检测寄存器,bit0:载波检�?
#define RX_ADDR_P0      0x0A	//数据通道0接收地址，最大长�?个字节，低字节在�?
#define RX_ADDR_P1      0x0B	//数据通道1接收地址，最大长�?个字节，低字节在�?
#define RX_ADDR_P2      0x0C	//数据通道2接收地址，最低字节可设置，高字节，必须同RX_ADDR_P1[39:8]相等
#define RX_ADDR_P3      0x0D	//数据通道3接收地址，最低字节可设置，高字节，必须同RX_ADDR_P1[39:8]相等
#define RX_ADDR_P4      0x0E	//数据通道4接收地址，最低字节可设置，高字节，必须同RX_ADDR_P1[39:8]相等
#define RX_ADDR_P5      0x0F	//数据通道5接收地址，最低字节可设置，高字节，必须同RX_ADDR_P1[39:8]相等
#define TX_ADDR         0x10	//发送地址(低字节在�?,ShockBurstTM模式下，RX_ADDR_P0与地址相等
#define RX_PW_P0        0x11	//接收数据通道0有效数据宽度(1~32字节),设置�?则非�?
#define RX_PW_P1        0x12	//接收数据通道1有效数据宽度(1~32字节),设置�?则非�?
#define RX_PW_P2        0x13	//接收数据通道2有效数据宽度(1~32字节),设置�?则非�?
#define RX_PW_P3        0x14	//接收数据通道3有效数据宽度(1~32字节),设置�?则非�?
#define RX_PW_P4        0x15	//接收数据通道4有效数据宽度(1~32字节),设置�?则非�?
#define RX_PW_P5        0x16	//接收数据通道5有效数据宽度(1~32字节),设置�?则非�?
#define NRF_FIFO_STATUS 0x17	//FIFO状态寄存器;bit0:RX FIFO寄存器空标志;bit1:RX FIFO满标�?bit2~3保留
								//bit4:TX FIFO 空标�?bit5:TX FIFO满标�?bit6:1,循环发送上一数据�?0,不循�?							
#define DYNPD			0x1C
#define FEATRUE			0x1D
//////////////////////////////////////////////////////////////////////////////////////////////////////////
