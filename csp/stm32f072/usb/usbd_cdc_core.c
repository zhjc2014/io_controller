...
extern kfifo_t vcp_rx_fifo;
extern kfifo_t vcp_tx_fifo;
...
uint8_t ALIGN(4) USB_Rx_Buffer   [CDC_DATA_MAX_PACKET_SIZE] ;

uint8_t ALIGN(4) APP_Rx_Buffer   [APP_RX_DATA_SIZE] ; 
...
{      
  ...
  //APP_FOPS.pIf_DataRx((uint8_t*)USB_Rx_Buffer, USB_Rx_Cnt);
  kfifo_push_in(&vcp_rx_fifo,USB_Rx_Buffer, USB_Rx_Cnt);
  /* Prepare Out endpoint to receive next packet */
  DCD_EP_PrepareRx(pdev,
                   CDC_OUT_EP,
                   (uint8_t*)(USB_Rx_Buffer),
