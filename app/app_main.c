...
static void set_io_status(int index)
{
    ...
    switch(index){
        ...
        for (i = 0; i < max_magician_per_line; i++){
            hal_app_write(DRV_GPIO, (uint8_t *)&io1_status, sizeof(uint32_t), gpio_magicain1_io1 + i, NULL);
            hal_app_write(DRV_GPIO, (uint8_t *)&io2_status, sizeof(uint32_t), gpio_magicain1_io2 + i, NULL);
        }
        break;
        case INDEX_1:    
        io1_status = 1;
